...
//funkcja zapisujaca dane do pliku ksiazki adresowej po dodaniu nowej osoby
void zapiszDaneNowejOsoby(Osoba osobaDoZapisu)
{
    fstream plikKsiazkaAdresowa;
    plikKsiazkaAdresowa.open("ksiazkaAdresowa.txt",ios::app);

    plikKsiazkaAdresowa<<osobaDoZapisu.idOsoby<<"|";
    plikKsiazkaAdresowa<<osobaDoZapisu.idUzytkownika << "|";
    plikKsiazkaAdresowa<<osobaDoZapisu.imie<<"|";
    plikKsiazkaAdresowa<<osobaDoZapisu.nazwisko<<"|";
    plikKsiazkaAdresowa<<osobaDoZapisu.telefon<<"|";
    plikKsiazkaAdresowa<<osobaDoZapisu.email<<"|";
    plikKsiazkaAdresowa<<osobaDoZapisu.adres<<"|"<<endl;

    plikKsiazkaAdresowa.close();
    return;
}

//funkcja zapisujaca dane do pliku ksiazki adresowej po edycji lub usunieciu osoby
int zapiszDaneEdytowanejOsoby(Osoba osobaDoZapisu, bool usunieta)
{
    fstream plikKsiazkaAdresowa;
    ...
    if(plikKsiazkaAdresowa.good()==true)
    {
        ...
        while(getline(plikKsiazkaAdresowa,aktualnaLiniaTekstu))
        {
            ...
            if(wczytywanaOsoba.idOsoby != osobaDoZapisu.idOsoby)
            {
            ...
            else
            {
                if (usunieta == false)
                {
                    ...
                    plikTymczasowy<<osobaDoZapisu.idUzytkownika << "|";
...
//funkcja wyznaczajaca najwysze id uzytkownika w ksiazce adresowej
...
int dodajOsobe(vector<Osoba> &ksiazkaAdresowa, int aktualneId, int idZalogowanegoUzytkownika )
{
    ...
    zapiszDaneNowejOsoby(osobaDoDodania);
    cout<<endl;
...
int usunOsobe(vector<Osoba> &ksiazkaAdresowa, int aktualneId)
{
    int id;
    ...
    if (osobaDoUsuniecia != ksiazkaAdresowa.end())
    {
        ...
        if(wyborUzytkownika=='t')
        {
            ...
            Sleep(3000);
            return aktualneIdOsoby;
        }
        else
        {
            int aktualneIdOsoby=aktualneId;
            return aktualneIdOsoby;
        }
    ...
    while(true)
    {
        ...
        else
        {
            ...
            case '4':
                ...
                aktualneIdOsoby=dodajOsobe(ksiazkaAdresowa, aktualneIdOsoby, idZalogowanegoUzytkownika);
            ...
            case '6':
                aktualneIdOsoby=usunOsobe(ksiazkaAdresowa, aktualneIdOsoby);
...
bool zapiszDaneNowejOsoby(Osoba osobaDoZapisu, int idZalogowanegoUzytkownika)
{
    fstream plikKsiazkaAdresowa;
    plikKsiazkaAdresowa.open("ksiazkaAdresowa.txt",ios::app);

    plikKsiazkaAdresowa<<osobaDoZapisu.idOsoby<<"|";
    plikKsiazkaAdresowa<< idZalogowanegoUzytkownika << "|";
    plikKsiazkaAdresowa<<osobaDoZapisu.imie<<"|";
    plikKsiazkaAdresowa<<(*osobaDoZapisu).nazwisko<<"|";
    plikTymczasowy<<osobaDoZapisu.telefon<<"|";
    plikTymczasowy<<osobaDoZapisu.email<<"|";
...
void zapiszDaneEdytowanejOsoby(Osoba osobaDoZapisu)
{
    fstream plikKsiazkaAdresowa;
    fstream plikTymczasowy;
    string aktualnaLiniaTekstu;
...
int zalogujUzytkownika(vector<Uzytkownik> uzytkownicy)
{
    ...
    int proby=0;
    cout<<endl;
    cout<<"Podaj nazwe uzytkownika:"<<end;
    cin>>nazwaUzytkownika;
    vector<Uzytkownik>::iterator uzytkownik;
    uzytkownik = find_if(uzytkownicy.begin(), uzytkownicy.end(), [nazwaUzytkownika]( const Uzytkownik &aktualnyUzytkownik){return aktualnyUzytkownik.nazwa==nazwaUzytkownika;});
    if (uzytkownik != uzytkownicy.end())
...
int dodajOsobe(vector<Osoba> &ksiazkaAdresowa, int aktualneId, int idZalogowanegoUzytkownika )
{
    ...
    ksiazkaAdresowa.push_back(osobaDoDodania);
    zapiszDaneNowejOsoby(osobaDoDodania, idZalogowanegoUzytkownika);
    cout<<endl;
    cout<<"Osoba zostala pomyslnie dodana do ksiazki adresowej.";
    Sleep(3000);
    return aktualneId;
...
void edytujOsobe(vector<Osoba> &ksiazkaAdresowa)
{
    ...
    if (osobaDoEdycji != ksiazkaAdresowa.end())
    {
        ...
        zapiszDaneEdytowanejOsoby((*osobaDoEdycji));
        cout<<endl;
        cout<<"Dane zmienione poprawnie"<<endl;
        Sleep(3000);
        return;
